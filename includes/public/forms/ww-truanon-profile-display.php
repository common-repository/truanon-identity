...
if (!empty($verify_result) && $verify_result['response']['code'] == '200' && $verify_result['response']['message'] == 'OK') {
  ...
  if ($user_data->type != "error") {
    ...
    $badgeReplacements = array($tru_user_name, $tru_user_title, $tru_rank_score, 'https://'.$user_profile_link, $tru_rank);
  }
}
...
if ($verify_status == "Confirm") {
  ...
  //$badgePatterns[]='{full_data}';
  //$badgeReplacements[]='<h3>$atts</h3><pre>'.print_r($atts,true).'</pre>'.'<h3>$user_data</h3><pre>'.print_r($user_data,true).'</pre>';
  $badgePatterns[]='{rank_color}';
  $badgeReplacements[]=$textClasses[strtolower($tru_rank)];
  echo str_replace($badgePatterns, $badgeReplacements, $badge1_description);
}
